...
import { useGalleryStore, loadLocalReactions, saveLocalReactions, loadUserReacted, saveUserReacted } from '@/store'
...
export default function Lightbox() {
  ...
  useEffect(() => {
    ...
    if (isLocal(photo)) {
      ...
      reactionAPI.get(photo._id, sessionId)
        .then(res => { setReactions(res.reactions || {}); setUserReacted(res.userReacted || {}) })
        .catch(() => {})
    }
  }, [index])
  ...
  const handleReact = async (emoji) => {
    ...
    if (isLocal(photo)) {
      ...
      const newCount = Math.max(0, (allRx[pid]?.[emoji] || 0) + (already ? -1 : 1))
      const newRx = { ...allRx, [pid]: { ...(allRx[pid] || {}), [emoji]: newCount } }
      const newUr = { ...allUr, [pid]: { ...(allUr[pid] || {}), [emoji]: !already } }
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      try { await navigator.clipboard.writeText(url); setShareMsg('Copied!'); setTimeout(() => setShareMsg(''), 2000) } catch {}
    }
  }
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(10,5,2,0.97)', WebkitTapHighlightColor: 'transparent' }}
    ...
    >
      ...
      <div className="flex-1 relative flex items-center justify-center min-h-0 px-2">
        {/* Prev arrow */}
        <button
          onClick={(e) => { e.stopPropagation(); prevPhoto(total) }}
        ...
        <img
          ...
          style={{ maxHeight: 'calc(100dvh - 210px)', transition: 'opacity 0.2s' }}
          draggable={false}
        />

        {/* Next arrow */}
        <button
          onClick={(e) => { e.stopPropagation(); nextPhoto(total) }}
      ...
      <div className="px-6 pt-2 pb-1 text-center flex-shrink-0">
        <p className="text-[#e8d8c4] italic text-base font-serif leading-snug">{photo.caption}</p>
      ...
      <div className="flex items-center justify-center gap-2 px-4 py-3 flex-shrink-0">
        {REACTION_EMOJIS.map((emoji) => {
          ...
          return (
            ...
            >
              ...
              <span className={`font-sans text-[10px] transition-colors ${active ? 'text-gold-300' : 'text-white/40'}`}>
                {count > 0 ? count : ''}
              </span>
            </button>
          )
        ...
        >
          ...
          <span className="font-sans text-[10px] text-white/40">{shareMsg || 'Share'}</span>
        </button>
      </div>
